...
def check_ranges_validity(
    dfI_discr_uniform, bins, continuous_attributes, verbose=False
):
    for cont_attr in continuous_attributes:
        if verbose:
            print(
                cont_attr,
                len(dfI_discr_uniform[cont_attr].value_counts()),
                dict(dfI_discr_uniform[cont_attr].value_counts()),
                sum(dfI_discr_uniform[cont_attr].value_counts().values),
            )
        if len(dfI_discr_uniform[cont_attr].value_counts()) != bins:
            if verbose:
                print(dict(dfI_discr_uniform[cont_attr].value_counts()))
            raise ValueError


def run_compas_experiments_uniform(
    ...
    outputdir = os.path.join(
        ouput_folder_dir, name_output_dir, dataset_name, "quantile"
    )
    ...
    for bins in [2, 3, 4, 5, 6, 7, 8]:
        ...
        check_ranges_validity(
            dfI_discr_uniform, bins, continuous_attributes, verbose=verbose
        )
        ...
        for min_sup_divergence in min_sup_divergences:
            ...
            filename = os.path.join(
                outputdir,
                f"stats_sdiv{min_sup_divergence}_uniform.json",
            )
